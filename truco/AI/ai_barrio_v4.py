...
from .ai_barrio_v4_params import *
...
class AiBarrioV4(AIInterface):
    ...
    def responder_envido(self, game_state: dict) -> dict:
        ...
        if envido_en_juego == 2:
            ...
            if envido_propio <= 20:
                ...
                probabilidad_de_querer = self.interpolate(envido_propio, 20, 25, RE_2_20, RE_2_25)
            elif envido_propio <= 29:
                probabilidad_de_querer = self.interpolate(envido_propio, 25, 29, RE_2_25, RE_2_29)
            else:
                probabilidad_de_querer = 1.0

            if self._probabilidad_a_decision(probabilidad_de_querer):
                if envido_propio <= 25:
                    probabilidad_de_subir = self.interpolate(envido_propio, 20, 25, SE_2_20, SE_2_25)
                elif envido_propio <= 29:
                    probabilidad_de_subir = self.interpolate(envido_propio, 25, 29, SE_2_25, SE_2_29)
                else:
                    probabilidad_de_subir = self.interpolate(envido_propio, 29, 33, SE_2_29, SE_2_33)
        ...
        # Envido, Envido
            ...
            if envido_propio <= 20:
                ...
                probabilidad_de_querer = self.interpolate(envido_propio, 20, 25, RE_4_20, RE_4_25)
            elif envido_propio <= 29:
                probabilidad_de_querer = self.interpolate(envido_propio, 25, 29, RE_4_25, RE_4_29)
            else:
                probabilidad_de_querer = self.interpolate(envido_propio, 29, 33, RE_4_29, RE_4_33)

            if self._probabilidad_a_decision(probabilidad_de_querer):
                if envido_propio <= 25:
                    probabilidad_de_subir = self.interpolate(envido_propio, 20, 25, SE_4_20, SE_4_25)
                elif envido_propio <= 29:
                    probabilidad_de_subir = self.interpolate(envido_propio, 25, 29, SE_4_25, SE_4_29)
                else:
                    probabilidad_de_subir = self.interpolate(envido_propio, 29, 33, SE_4_29, SE_4_33)
        ...
        # Envido, Envido, Real Envido
            ...
            if envido_propio <= 20:
                ...
                probabilidad_de_querer = self.interpolate(envido_propio, 20, 25, RE_7_20, RE_7_25)
            elif envido_propio <= 29:
                probabilidad_de_querer = self.interpolate(envido_propio, 25, 29, RE_7_25, RE_7_29)
            else:
                probabilidad_de_querer = self.interpolate(envido_propio, 29, 33, RE_7_29, RE_7_33)
    ...
    def _envido_propio_a_probabilidades(self, puntos: int, soy_mano: bool) -> float:
        if soy_mano:
            if puntos <= 7:
                ...
                return self.interpolate(puntos, 20, 25, CE_M_20, CE_M_25)
            elif puntos <= 29:
                return self.interpolate(puntos, 25, 29, CE_M_25, CE_M_29)
            else:
                return self.interpolate(puntos, 29, 33, CE_M_29, CE_M_33)
        else:
            if puntos <= 7:
                return CE_NM_0_7
            elif puntos <= 25:
                return self.interpolate(puntos, 20, 25, CE_NM_20, CE_NM_25)
            elif puntos <= 29:
                return self.interpolate(puntos, 25, 29, CE_NM_25, CE_NM_29)
            else:
                return self.interpolate(puntos, 29, 33, CE_NM_29, CE_NM_33)
    ...
    def responder_truco(self, game_state: dict) -> dict:
        ...
        if len(bazas) == 0:
            ...
            if [a for a in acciones if a == "envido" or a == "real_envido" or a == "falta_envido"]:
                ...
                if self._probabilidad_a_decision(prob_cantar_envido):
                    if self._probabilidad_a_decision(RT_PROB_REAL_ENVIDO):
                        return {"tipo": "real_envido"}
                    else:
                        return {"tipo": "envido"}

        if chance_de_ganar >= RT_UMBRAL_SUBIR:
            if len(mano) != 0:
        ...
        if chance_de_ganar <= RT_UMBRAL_NO_QUERER:
            return {"tipo": "no_quiero"}
    ...
    def _elegir_primera_carta(self, mano) -> dict:
        mano_ordenada = sorted(mano, key=lambda c: c.get("poder", 0))
        if self._probabilidad_a_decision(EP1_PROB_ALTA):
            print("Elijo la carta mas alta")
            return mano_ordenada[2]
        elif self._probabilidad_a_decision(EP1_PROB_BAJA_COND):
    ...
    def _elegir_segunda_carta(self, mano) -> dict:
        ...
        elif self._probabilidad_a_decision(ES2_PROB_ALTA):
            print("Elijo la carta mas alta")
            return mano_ordenada[1]
        else:
            print("Elijo la carta mas baja")
    ...
    def canto_truco(self, game_state: dict) -> bool:
        ...
        if len(bazas) == 0:
            ...
            if chance_de_ganar <= CT_BLUFF_INF or chance_de_ganar >= CT_BLUFF_SUP:
                return self._probabilidad_a_decision(CT_PROB_BLUFF)
            ...
            if bazas[0] == "oponente" and chance_de_ganar >= CT_UMBRAL_GANAR_TRAS_PERDER:
                return True

            # La mejor situación para correrlo si no me queda nada
            if bazas[0] == "yo" and chance_de_ganar <= CT_UMBRAL_BLUFF_GANAR:
                return True
            ...
            return self._probabilidad_a_decision(chance_de_ganar - CT_OFFSET_PROP)
            ...
            if soy_mano:
                ...
                if chance_de_ganar >= 0.5:  # Si está bien la logica aca la chance es 0.0 o 1.0
                    ...
                    return self._probabilidad_a_decision(CT_PROB_COIN_FLIP)
